...
const feriedager = (meldekort) => {
  if (meldekort.resterendeFeriedager && meldekort.resterendeFeriedager > 0) {
    return <F id="meldekort.feriedager" values={{ feriedager: meldekort.resterendeFeriedager }} />;
  }
  return null;
};
...
const trekk = (skalViseTrekkdato, formatDateMonth, next) => {
  if (skalViseTrekkdato) {
    return <F id="meldekort.info.om.trekk" values={{ dato: formatDateMonth(next.sisteDatoForTrekk) }} />;
  }
  return null;
};
